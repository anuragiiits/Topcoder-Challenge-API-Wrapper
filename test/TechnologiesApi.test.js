  ...
  describe(`Technology API Tests (${clientName})`, () => {
    ...
    describe('Test create technology', () => {
      it(`create technology success`, async () => {
        ...
        createdTechnologyId = res.body.result.content.id
        should.equal(res.status, 200)
        should.equal(res.body.result.content.name, "J2EE")
        should.equal(res.body.result.content.description, "Java 2 Enterprise Edition")
        should.equal(res.body.result.content.status.id, 1)
        should.equal(res.body.result.content.status.description, "Active")
      })
    ...
    describe('Test patch technology by id', () => {
      it(`patch technology by id success`, async () => {
        const res = await client.patchTechnology(createdTechnologyId, {
				...
				should.equal(res.status, 200)
				should.equal(res.body.result.content.id, createdTechnologyId)
        should.equal(res.body.result.content.name, "Python")
        should.equal(res.body.result.content.description, "Python OOPS")
        should.equal(res.body.result.content.status.id, 1)
        should.equal(res.body.result.content.status.description, "Active")
      })
